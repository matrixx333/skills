...
description: Write integration tests using .NET Aspire's testing facilities with NUnit. Covers test fixtures, distributed application setup, endpoint discovery, and patterns for testing ASP.NET Core apps with real dependencies.
invocable: false
---

# Integration Testing with .NET Aspire + NUnit
...
3. **Fixture Lifecycle** - Use `[OneTimeSetUp]` / `[OneTimeTearDown]` for proper test fixture setup and teardown
4. **Endpoint Discovery** - Never hard-code URLs; discover endpoints from Aspire at runtime
5. **Parallel Isolation** - NUnit runs serially by default; only opt in with `[Parallelizable]` where tests are truly isolated
...
```
┌─────────────────┐                    ┌──────────────────────┐
│ NUnit test file │──uses────────────►│  AspireFixture       │
└─────────────────┘                    │  ([SetUpFixture])    │
                                       └──────────────────────┘
                                               │
                                               │ starts
...
[SetUpFixture]
public sealed class AspireAppFixture
{
    private static DistributedApplication? _app;

    public static DistributedApplication App => _app
    ...
    public static Uri GetEndpoint(string resourceName, string scheme = "https")
    {
...
```csharp
// The [SetUpFixture] above runs once for every fixture in the same namespace,
// so tests just reach for the shared application.
...
public class IntegrationTests
{
    ...
    {
        var httpClient = AspireAppFixture.App.CreateHttpClient("yourapp");
...
| Port conflicts between tests | Use a NUnit `[SetUpFixture]` to share one app instance across the namespace |
...
| Parallel tests interfere | Leave fixtures unmarked (NUnit is serial by default) or add `[NonParallelizable]` |
| Aspire dashboard conflicts | Only one dashboard can run at a time; tests reuse the same instance |

## Best Practices

1. **Use `[OneTimeSetUp]` / `[OneTimeTearDown]`** - Ensures proper async initialization and cleanup
2. **Share fixtures via `[SetUpFixture]`** - Reduces test execution time by reusing app instances
