...
description: Choosing the right concurrency abstraction in .NET - from async/await for I/O to Channels for producer/consumer to Reactive Extensions for event composition. Avoid locks and manual synchronization unless absolutely necessary.
invocable: false
---
...
- Need to process streams of data with backpressure or debouncing
...
1. **First choice:** Redesign to avoid it (immutability, message passing, state isolation)
...
## Level 4: Reactive Extensions (Event Composition)

For advanced scenarios requiring UI event composition, see [advanced-concurrency.md](advanced-concurrency.md).

**Reactive Extensions** are ideal for composing event streams: debouncing search-as-you-type, throttling auto-save, and combining multiple event sources.
...
// GOOD: Use a Channel to serialize access
```
...
**Only escalate when you have a concrete need.** Don't reach for heavier abstractions "just in case".
